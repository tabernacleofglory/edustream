...
import { Loader2, Plus, Trash2, X, Combine } from "lucide-react";
...
const formFieldSchema = z.object({
  ...
  type: z.enum(["text", "email", "phone", "textarea", "select", "multiple-choice", "multiple-select", "password"]),
  options: z.array(z.string()).optional(),
  dataSource: z.enum(["manual", "campuses"]).optional().default("manual"),
...
const formBuilderSchema = z.object({
  ...
  type: z.enum(['blank', 'hybrid']).default('blank'),
});
...
function BlankFormBuilder() {
    ...
    const formType = searchParams.get('type') as 'hybrid' | 'blank' | null;
        ...
        const fetchForm = async () => {
            if (formId) {
                ...
                if (docSnap.exists()) {
                 ...
                 form.reset({
                    title: formType === 'hybrid' ? 'New Hybrid Form' : 'Untitled Form',
    ...
    const handleQuestionTypeChange = (value: "text" | "email" | "phone" | "textarea" | "select" | "multiple-choice" | "multiple-select" | "password", index: number) => {
        ...
        <div className="space-y-8">
             <div>
                <h1 className="font-headline text-3xl font-bold md:text-4xl flex items-center gap-2">
                  {formType === 'hybrid' && <Combine />}
                  {formId ? 'Edit Form' : formType === 'hybrid' ? 'Hybrid Form Builder' : 'Blank Form Builder'}
                </h1>
                <p className="text-muted-foreground">
                    {formId ? 'Edit your custom form.' : `Create a new ${formType || 'blank'} form.`}
                </p>
            </div>
             <Form {...form}>
