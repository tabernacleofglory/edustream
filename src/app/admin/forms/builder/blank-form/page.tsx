...
type Opt = { value: string; label: string, id?: string };
...
const formBuilderSchema = z.object({
  ...
  customRedirectEnabled: z.boolean().default(false),
...
function MobilePreview({ form, dynamicOptions }: { form: any, dynamicOptions: Record<string, Opt[]> }) {
    ...
    const getOptionsForField = (field: any) => {
        ...
        let opts = dynamicOptions[field.dataSource] || [];
        
        // Apply data source filtering if set
        if (field.dataSource === 'ladders' && field.dataSourceOptions?.ladders?.length) {
            opts = opts.filter(o => field.dataSourceOptions.ladders.includes(o.id));
        }
        if (field.dataSource === 'campuses' && field.dataSourceOptions?.campuses?.length) {
            opts = opts.filter(o => field.dataSourceOptions.campuses.includes(o.id));
        }
...
function BlankFormBuilder() {
    ...
    const [editingDataSourceOptions, setEditingDataSourceOptions] = useState<{ fieldIndex: number, type: 'ladders' | 'campuses' } | null>(null);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                                                        ...
                                                        {(form.watch(`fields.${index}`) as any)._showDataSource && (
                                                            <div className="mt-2 space-y-2 bg-muted/50 p-2 rounded-md">
                                                                ...
                                                                <div className="flex gap-1 items-center">
                                                                    ...
                                                                    {(dataSource === 'ladders' || dataSource === 'campuses') && (
                                                                        <Button 
                                                                            type="button" 
                                                                            variant="outline" 
                                                                            size="icon" 
                                                                            className="h-7 w-7 shrink-0" 
                                                                            onClick={() => setEditingDataSourceOptions({ fieldIndex: index, type: dataSource })}
                                                                        >
                            ...
                            <CardContent className="p-0">
                                <Table>
                                    <TableBody>
                                        ...
                                        <TableRow>
                                            <TableCell className="font-medium">Redirect Post-Submission</TableCell>
                                            <TableCell className="space-y-4">
                                                <div className="flex items-center gap-2">
                                                    <Switch
                                                        checked={form.watch('customRedirectEnabled')}
                                                        onCheckedChange={(checked) => form.setValue('customRedirectEnabled', checked)}
                                                    />
                                                    <Label className="text-xs">Redirect after submission</Label>
                                                </div>
                                                {form.watch('customRedirectEnabled') && (
                                                    <div className="pl-8 pt-2">
                                                        <Select 
                                                            onValueChange={(v) => form.setValue('redirectPath', v)} 
                                                            value={form.watch('redirectPath')}
                                                        >
                                                            <SelectTrigger>
                                                        ...
                                                        <p className="text-[10px] text-muted-foreground mt-2">Users will be automatically sent to this page after submitting.</p>
                                                    </div>
                                                )}
                                            </TableCell>
                                        </TableRow>
            ...
            {editingDataSourceOptions && (
                <Dialog open={!!editingDataSourceOptions} onOpenChange={() => setEditingDataSourceOptions(null)}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Filter {editingDataSourceOptions.type === 'ladders' ? 'Ladders' : 'Campuses'}</DialogTitle>
                            <DialogDescription>Select which items to show in the dropdown. If none are selected, all will be shown.</DialogDescription>
                        </DialogHeader>
                        <ScrollArea className="h-64 border rounded-md p-4">
                            <div className="space-y-2">
                                {(editingDataSourceOptions.type === 'ladders' ? allLadders : allCampuses).map(item => {
                                    const path = `fields.${editingDataSourceOptions.fieldIndex}.dataSourceOptions.${editingDataSourceOptions.type}`;
                                    const currentValues = form.watch(path as any) || [];
                                    const isChecked = currentValues.includes(item.id || item.value);
                                    
                                    return (
                                        <div key={item.id || item.value} className="flex items-center space-x-2">
                                            <Checkbox 
                                                id={`ds-opt-${item.id || item.value}`} 
                                                checked={isChecked}
                                                onCheckedChange={(checked) => {
                                                    const next = checked 
                                                        ? [...currentValues, item.id || item.value]
                                                        : currentValues.filter((v: string) => v !== (item.id || item.value));
                                                    form.setValue(path as any, next);
                                                }}
                                            />
                                            <Label htmlFor={`ds-opt-${item.id || item.value}`}>{item.label}</Label>
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <DialogFooter>
                            <Button onClick={() => setEditingDataSourceOptions(null)}>Done</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            )}
