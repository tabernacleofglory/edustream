...
import { LayoutDashboard, BookOpen, UserCog, Settings, LogOut, Tv, Music, MessageSquare, BookCopy, Shield, Menu, Award, Theater, Play, AlertTriangle, Home } from "lucide-react";
...
function AppContentInternal({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "siteSettings", "main"), (docSnap) => {
       if (docSnap.exists()) {
           setMaintenanceModeEnabled(!!docSnap.data().maintenanceModeEnabled);
       }
    });
    return () => unsub();
  }, [db]);
  ...
  if (maintenanceModeEnabled && !loading && user && !hasPermission('bypassMaintenance')) {
    return (
       <div className="min-h-screen flex items-center justify-center bg-background p-4 flex-col text-center space-y-6">
           <AlertTriangle className="h-16 w-16 text-destructive" />
           <h1 className="text-4xl font-bold font-headline">{t('maintenance.title', 'Under Maintenance')}</h1>
           <p className="text-muted-foreground max-w-md">
               {t('maintenance.description', 'The platform is currently undergoing scheduled maintenance. Please check back later. We apologize for the inconvenience.')}
           </p>
           <div className="flex gap-4 items-center">
               <Button asChild className="bg-gradient-to-r from-blue-900 to-gray-400 text-white hover:from-blue-800 hover:to-gray-300 border-0">
                   <Link href="/">
                       <Home className="mr-2 h-4 w-4" /> {t('nav.home', 'Go to Home')}
                   </Link>
               </Button>
               <Button variant="destructive" onClick={async () => { await getFirebaseAuth().signOut(); router.push('/'); }}>
