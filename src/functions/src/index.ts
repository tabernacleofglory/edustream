...
export const syncUserGlobalProgress = functions
  ...
  .https.onCall(async (data: any, context: functions.https.CallableContext) => {
    ...
    try {
      ...
      // 3. Get manual on-site credits
      const onsiteSnap = await db.collection("onsiteCompletions").where("userId", "==", userId).get();
      onsiteSnap.forEach(doc => {
          const d = doc.data();
          if (d.courseId) legacyCompletions.add(d.courseId);
          if (Array.isArray(d.creditedVideos)) d.creditedVideos.forEach((id: string) => legacyCompletions.add(id));
          if (Array.isArray(d.creditedQuizzes)) d.creditedQuizzes.forEach((id: string) => legacyCompletions.add(id));
          if (d.creditedForm?.formId) legacyCompletions.add(d.creditedForm.formId);
      });

      // 4. Get completed enrollments
      const enrollSnap = await db.collection("enrollments").where("userId", "==", userId).get();
      enrollSnap.forEach(doc => {
          const d = doc.data();
          if (d.completedAt) legacyCompletions.add(d.courseId);
      });
      ...
      if (migratedCount > 0) {
          await globalProgressRef.set(itemsToMigrate, { merge: true });
      }
      
      return { success: true, migratedCount };
    } catch (error) {
      console.error("Global sync function error:", error);
