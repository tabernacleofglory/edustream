...
export default function PublicBlankForm({ formConfig, courseId, existingSubmission, onFormComplete }: PublicBlankFormProps) {
  ...
  useEffect(() => {
    if (existingSubmission) {
        form.reset(existingSubmission.data || existingSubmission);
    } else if (user) {
  ...
  }, [formConfig.id, form, user?.uid, formConfig.fields, existingSubmission]);

  useEffect(() => {
    if (!user && !existingSubmission) {
        localStorage.setItem(`form-draft-${formConfig.id}`, JSON.stringify(debouncedData));
    }
  }, [debouncedData, formConfig.id, user?.uid, existingSubmission]);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (user?.uid) {
          // One Source of Truth: Write to Global Progress map
          const globalProgressRef = doc(db, 'userContentProgress', user.uid);
          await setDoc(globalProgressRef, { 
              completedItems: { [formConfig.id]: serverTimestamp() } 
          }, { merge: true });
          ...
          <CardHeader className="text-center">
            ...
            {existingSubmission && (
                <Badge variant="secondary" className="mx-auto mt-2">
                    Response from {format(existingSubmission.submittedAt?.toDate() || new Date(), 'PP')}
                </Badge>
            )}
          </CardHeader>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <CardFooter>
              <Button type="submit" className="w-full" disabled={isSubmitting || loadingOptions || !!existingSubmission}>
                ...
                {existingSubmission ? "Already Submitted" : "Submit Response"}
              </Button>
            </CardFooter>
          </form>
        </Card>
