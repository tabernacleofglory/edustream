...
export default function PermissionManager() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Show developer first (read-only column)
        ...
        // Ensure every role has a key in state (even if no doc yet)
        rolesList.forEach(role => {
          if (!permissionsData[role]) permissionsData[role] = [];
        });
        // Developer has everything by definition (read-only in UI)
  ...
  const handlePermissionChange = (role: string, permissionId: string, checked: boolean) => {
    setPermissions(prev => {
      const next = { ...prev };
      const rolePerms = next[role] ? [...next[role]] : [];
      if (checked) {
        if (!rolePerms.includes(permissionId)) rolePerms.push(permissionId);
      } else {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const writes = Object.entries(permissions).map(([role, perms]) => {
        ...
        return setDoc(ref, { role, permissions: perms });
      });
      await Promise.all(writes.filter(Boolean) as Promise<any>[]);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>You do not have the required permissions to view or manage role permissions.</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert variant="destructive">
            <Lock className="h-4 w-4" />
            <AlertTitle>Permission Required</AlertTitle>
            <AlertDescription>This section requires the ‘Manage Permissions’ permission.</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Assign permissions to roles. Developer has all permissions by default (read-only).</CardDescription>
      </CardHeader>
      <CardContent>
