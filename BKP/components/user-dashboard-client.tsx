    ...
    useEffect(() => {
        ...
        const fetchFullCurriculum = async () => {
            ...
            try {
                ...
                const quizDone = new Set(quizResultsSnap.docs.map(d => d.data().quizId));
                const globalDone = new Set(globalSnap.exists() ? Object.keys(globalSnap.data().completedItems || {}) : []);
                const formDone = new Set(formSubmissionsSnap.docs.map(d => d.data().formId));
                    ...
                    if (videoIds.length > 0) {
                        ...
                        videoIds.forEach(id => {
                            allItems.push({ 
                                ...
                                id, 
                                isCompleted: videoDone.has(id) || globalDone.has(id)
                            });
                        });
                    }
                    ...
                    if (quizIds.length > 0) {
                        ...
                        quizIds.forEach(id => {
                            allItems.push({ 
                                ...
                                id, 
                                isCompleted: quizDone.has(id) || globalDone.has(id)
                            });
                        });
                    }
                    ...
                    if (formId) {
                        ...
                        if (fSnap.exists()) {
                            allItems.push({ 
                                ...
                                id: fSnap.id, 
                                isCompleted: formDone.has(formId) || globalDone.has(formId)
                            });
                        }
                    }

                    // Only add courses that have missing items
