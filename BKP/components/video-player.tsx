          ...
          <AccordionContent className="px-4 space-y-2">
            {quizzes.map((quiz, index) => {
              ...
              const prevQuizPassed = prevQuizId
                ? quizResults.some((r) => r.quizId === prevQuizId && r.passed)
                : true;
          ...
          <AccordionContent className="px-4 space-y-2">
            {forms.map((form) => {
              const allQuizzesCompleted = (course.quizIds || []).every((qid) =>
                quizResults.some((r) => r.quizId === qid && r.passed)
              );
              const isFormLocked = !lastVideoCompleted || !allQuizzesCompleted;
              const isSubmitted = formSubmissions.some(
...
export default function VideoPlayerClient({
    ...
    const unsubscribeProgress = onSnapshot(progressRef, (progressSnap) => {
      onSnapshot(globalProgressRef, (globalSnap) => {
        const courseSpecificCompleted = new Set<string>(
          progressSnap.exists()
            ? (progressSnap.data() as UserProgressType).videoProgress
      ...
      try {
        ...
        if (completed) {
          ...
          const allVideosCompleted = publishedVideoIds.every((vid) =>
            currentProgress.some((p) => p.videoId === vid && p.completed)
          );
          ...
          const allFormsCompleted = (forms || []).every((form) =>
            formSubmissions.some(
              (sub) => sub.formId === form.id && sub.courseId === course.id
            )
          );
  ...
  useEffect(() => {
    ...
    const onSeeked = () => {
      if (v && v.currentTime > farthestTimeWatchedRef.current) {
  ...
  const handleEnded = async () => {
    ...
    const currentDuration =
      isYouTube || isGoogleDrive
        ? reactPlayerRef.current?.getDuration() ?? 0
        : playerRef.current?.duration ?? duration;
  ...
  const handleShare = async () => {
    ...
    const recordShare = async () => {
      if (!user) return;
      await addDoc(collection(db, "Contents", currentVideo.id, "shares"), {
        uid: user.uid,
  ...
  const handleFullScreen = () => {
    ...
    if (!document.fullscreenElement) {
      playerContainer
        .requestFullscreen()
        .catch((err) => alert(`Error enabling full-screen: ${err.message}`));
    } else {
  ...
  const maybeTriggerStillWatchingPrompt = useCallback(
    (watchedSeconds: number, totalDuration: number) => {
      if (!currentVideo.enableStillWatchingPrompt) return;
      if (!totalDuration || totalDuration < STILL_WATCHING_INTERVAL_SECONDS) return;

      const checkpoint = Math.floor(watchedSeconds / STILL_WATCHING_INTERVAL_SECONDS);

      if (checkpoint >= 1 && !promptCheckpointsRef.current.has(checkpoint)) {
        setIsPlaying(false);
        setIsStillWatchingPromptVisible(true);
        promptCheckpointsRef.current.add(checkpoint);
      }
    },
    [currentVideo.enableStillWatchingPrompt]
  );
  ...
  const handlePlayerProgress = (state: {
    ...
    maybeTriggerStillWatchingPrompt(t, d);
  ...
  return (
    ...
    >
      <div className={cn("flex-1 flex flex-col lg:h-screen", currentTrack && "pb-16 lg:pb-0")}>
        <div className="lg:px-8 lg:pt-8 flex-shrink-0">
          ...
          >
            {isYouTube || isGoogleDrive ? (
              ...
              <video
                ...
                onTimeUpdate={(e) => {
                  ...
                  const d =
                    target.duration && Number.isFinite(target.duration)
                      ? target.duration
                      : duration;
            ...
            {!isPlaying && !showEndOverlay && (
              <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-20 w-20 bg-black/50 text-white hover:bg-black/70 hover:text-white pointer-events-auto"
                  onClick={togglePlayPause}
                >
            ...
            <div
              className={cn(
                "video-controls absolute bottom-0 left-0 right-0 z-20 p-2 md:p-4 bg-gradient-to-t from-black from-10% via-black/70 to-transparent transition-opacity",
                  ...
                  if (newTime > farthestTimeWatchedRef.current) {
                    ...
                    if (isYouTube || isGoogleDrive) {
                      reactPlayerRef.current?.seekTo(
                        farthestTimeWatchedRef.current,
                        "seconds"
                      );
                    } else if (playerRef.current) {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {Math.max(0, course.enrollmentCount || 0)} Learners
                    </p>
                  </div>
                </div>
      ...
      <Dialog
        open={isStillWatchingPromptVisible}
        onOpenChange={setIsStillWatchingPromptVisible}
      >
        <DialogContent onInteractOutside={(e) => e.preventDefault()}>
          ...
          <DialogFooter>
            <Button
              onClick={() => {
                setIsStillWatchingPromptVisible(false);
                setIsPlaying(true);
              }}
            >
              Yes, I am
