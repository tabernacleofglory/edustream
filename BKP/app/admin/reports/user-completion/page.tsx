...
export default function UserCompletionReport() {
  ...
  // High-performance data maps for the main list summary
    ...
    try {
      ...
      usersSnap.docs.forEach(uDoc => {
          ...
          coursesList.forEach(c => {
              ...
              const videosOk = (c.videos || []).every(id => myVideoDone.has(id));
              const quizzesOk = (c.quizIds || []).every(id => myPassedQuizzes.has(id));
              if(videosOk && quizzesOk && ((c.videos?.length || 0) > 0 || (c.quizIds?.length || 0) > 0)) {
                  addComp(uid, c.id);
              }
          });
      });
  ...
  const handleUserClick = async (user: any) => {
    ...
    try {
        ...
        courses.forEach(c => {
            const videosOk = (c.videos || []).every(id => videoDone.has(id));
            const quizzesOk = (c.quizIds || []).every(id => quizzesDone.has(id));
            const formOk = !c.formId || formsDone.has(c.formId);
            if (videosOk && quizzesOk && formOk && ((c.videos?.length || 0) > 0 || (c.quizIds?.length || 0) > 0 || c.formId)) {
